...
const linuxUnitName = "veil.service"
...
func (linuxManager) Plan(action Action, opts Options) (*Plan, error) {
	...
	} else if err := requireInstalled(unitPath); err != nil {
		return nil, err
	}
...
func buildSystemdUnit(opts Options) string {
	args := proxyArgs(opts)
	var b strings.Builder
	_, _ = b.WriteString("[Unit]\n")
	_, _ = b.WriteString("Description=Veil local de-identification proxy\n")
	_, _ = b.WriteString("After=network.target\n\n")
	_, _ = b.WriteString("[Service]\n")
	_, _ = b.WriteString("Type=simple\n")
	_, _ = b.WriteString("ExecStart=" + systemdEscape(opts.BinaryPath))
	for _, arg := range args {
		_, _ = b.WriteString(" " + systemdEscape(arg))
	}
	_, _ = b.WriteString("\n")
