...
- **Streaming tool arguments**: `response.function_call_arguments.delta` and related delta streams can split `PAIArtVeil_` tokens or format-preserving surrogates across events; arguments are buffered and restored before local tool execution sees them. Verified by: stream_test.go.
