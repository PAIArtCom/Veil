...
func TestStreamTextDeltaInPlace(t *testing.T) {
	restore := mapRestore(map[string]string{"CLK_SECRET_0a1b2c3d4e5f": "SECRET"})
	out := collect(t, restore,
		[]byte(`{"type":"content_block_start","index":0,"content_block":{"type":"text"}}`),
		[]byte(`{"type":"content_block_delta","index":0,"delta":{"type":"text_delta","text":"x CLK_SECRET_0a1b2c3d4e5f y"}}`),
...
func TestStreamTextDeltaHeldThenFlushed(t *testing.T) {
	...
	// "CLK_SE" is wholly a partial-token prefix → entire delta held.
	outs, err := sr.Event([]byte(`{"type":"content_block_delta","index":0,"delta":{"type":"text_delta","text":"CLK_SE"}}`), restore)
	if err != nil {
...
func TestStreamToolConsolidation(t *testing.T) {
	restore := mapRestore(map[string]string{"CLK_SECRET_0a1b2c3d4e5f": "real-dsn"})
	p := &provider{}
	sr, _ := p.NewStreamRestorer("messages")
	...
	o1, _ := sr.Event([]byte(`{"type":"content_block_delta","index":1,"delta":{"type":"input_json_delta","partial_json":"{\"dsn\":\"CLK_SEC"}}`), restore)
