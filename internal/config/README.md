...
`internal/config` is the local policy source for the OpenCloak OSS binary.

## Purpose

This module loads a single-user local policy file, validates it fail-closed, and
exposes it as the `PolicyProvider` used by `cmd/opencloak`. It is the OSS default
...
- Does NOT handle: Transport startup, provider routing, or credential forwarding (see: ../../cmd/opencloak/README.md and ../proxy/README.md).
...
- **Startup fallback confusion**: A configured but invalid policy file must block startup; only an absent default file falls back to the built-in policy. Verified by: policy_test.go and ../../cmd/opencloak/main_test.go.
