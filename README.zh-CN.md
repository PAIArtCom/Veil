...
> **状态：Claude Code 代理路径已通过 Phase 0 验收。** 文本引擎、Anthropic Messages
> wire 掩码/还原、流式还原、以及本地 Claude Code 代理已实现、通过测试验证，并已
> 通过真实 Claude Code 流量验收。见 [Phase 0 验收报告](docs/architecture/phase-0-acceptance.md)。
...
1. **独立本地代理** —— 把 CLI 的 base URL 指向它（Claude Code 用 `ANTHROPIC_BASE_URL`；
   Codex 计划后续通过自定义 `model_providers` 条目支持）。凭证原样透传，只改写请求体。
2. **可嵌入 Go 库** —— 把引擎放进你自己的网关，在你的请求/响应接缝处调用。SDK 是
   **通用的**，以多个真实网关验证设计 —— 不为任何单一网关定制。见 [SDK 契约](docs/sdk/contract.md)。
