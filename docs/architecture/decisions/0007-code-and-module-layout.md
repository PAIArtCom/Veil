...
- Module path **`github.com/PAIArtCom/Veil`** (PAIArt is the GitHub org; Veil is
  the open-source repository under it).
- The **public SDK lives at the module root package** (`veil`).
- A **single binary with subcommands** (`veil proxy|serve|console|mask`).
- **PAIArt is a separate private repository.**
- L1 detection merges **both** rule sources (privacy-filter + gitleaks).
- A **local, single-user web console** belongs in Veil (open); the multi-tenant
  management/audit console belongs in PAIArt.

## Decision

The root package `veil` is the public API (the `Engine` + Text/Wire/Stream methods +
the extension interfaces). All implementation lives under `internal/`. Transports and the
CLI live under `cmd/veil`.

```
veil/                       module github.com/PAIArtCom/Veil
├── veil (root package)     PUBLIC API
...
└── cmd/veil/     single binary: proxy | serve | console | mask
```

Two extension interfaces are the **seams** between open and commercial code:

- `PolicyProvider` — local-file default in OSS; PAIArt pushes/hot-reloads scoped central
  policy.
- `AuditSink` — no-op/local default in OSS; PAIArt collects minimized audit data.

The **local web console** and PAIArt's **org console** sit on these same two seams (local
...
- **Public API under `pkg/`.** Rejected: a root-package import
  (`github.com/PAIArtCom/Veil`) is cleaner and idiomatic; `internal/` already hides
  implementation.
- **Multiple binaries.** Rejected: one binary with subcommands is simpler to distribute.
- **Monorepo including PAIArt.** Rejected: commercial code stays out of the Apache-2.0
  repo; PAIArt depends on Veil and implements the two interfaces.
- **Public third-party provider adapter registry in Phase 0.** Rejected for now: the SDK
  contract must be stable first, and Veil should prove the loop with maintained
...
- The engine is importable as `github.com/PAIArtCom/Veil`; gateways embed it directly.
- The OSS repo never imports commercial code; the dependency points one way (PAIArt →
  Veil).
